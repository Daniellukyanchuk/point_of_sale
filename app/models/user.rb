class User < ApplicationRecord
  ...
  def update_role_users(user_record)
    current_roles = RoleUser.where("user_id = ?", user_record[:id].to_i)
    RoleUser.delete(current_roles)
    incoming_values = user_record[:role][:role_user_attributes][:role_user_id].values
